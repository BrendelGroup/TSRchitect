...
```{r eval=FALSE}
Hs_RAMPAGE <- loadTSSobj(expTitle ="Human RAMPAGE", inputDir="HsRAMPAGEbam/", /
inputType="bam", isPairedEnd=TRUE, sampleNames=c("H460-rep1", /
...
```{r eval=FALSE}
Hs_RAMPAGE <- bamToTSS(experimentName=Hs_RAMPAGE)
```
...
```{r eval=FALSE}
require(ggplot2)
HT1080.tsrs.filtered <- HT1080.tsrs[HT1080.tsrs$nTSSs > 100,]
t <- ggplot(HT1080.tsrs.filtered, aes(shapeIndex))
...
```{r eval=FALSE}
H460.tsrs.filtered <- H460.tsrs[H460.tsrs$nTSSs > 100,]
p <- ggplot(H460.tsrs.filtered, aes(shapeIndex))
...
Similarly, we could use the same code framework above to plot the distributions of TSR width and/or nTSSs; to do this we would select the appropriate column.
...
```{r eval=FALSE}
At_PEAT <- loadTSSobj(expTitle ="Arabidopsis PEAT dataset", inputDir="PEATbam", /
inputType="bam", isPairedEnd=TRUE, sampleNames=c("experiment1"), replicateIDs=c(1))
```

Now we convert the alignment data (in this case, in .bam format) into TSS coordinates:

```{r eval=FALSE}
At_PEAT <- bamToTSS(At_PEAT)
```
...
inputType="bam", isPairedEnd=TRUE, sampleNames=c("A549-rep1","A549-rep2", /
...
CAGEhuman <- bamToTSS(experimentName=CAGEhuman)
```
