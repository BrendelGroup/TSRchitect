...
tsrToDF <- function(x) {
    len.list <- length(x)
    chr.vec <- names(x)
    final.matrix <- matrix(NA, nrow=1, ncol=6)
    ...
    for (j in 1:len.list) {
        ...
        matrix(NA, nrow=length(this.p), ncol=6) -> plus.matrix
        ...
        for (i in 1:length(this.p)) {
            ...
            c(this.chr, my.start, my.end, my.strand, my.width, my.SI) -> my.string
            my.string -> plus.matrix[i,]
        }
            this.chr.list$minus -> this.m
            c("-") -> my.strand
            #matrix(NA, nrow=length(this.m), ncol=7) -> minus.matrix
            matrix(NA, nrow=length(this.m), ncol=6) -> minus.matrix
    ...
    colnames(final.matrix) <- c("chr", "start", "end", "strand", "tsrWidth", "shapeIndex")
