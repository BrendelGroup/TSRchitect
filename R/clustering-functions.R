...
expressionTSS <- function(x, dfName="TSS.txt", writeDF=FALSE) {
...
#VB Note: Print a progress note on every 10th sequence; 10 should be a parameter
            if (i%%10 == 0) {
                cat("\n... expressionTSS running with sequence ", i, " of ", n.chr, "\n")
            }
            uni.chr[i] -> this.chr
...
#' returns a list of TSRs from the data.frame generated by expressionTSS()
#' @export
