...
In your current directory, create subdirectories as follows:
```{bash eval=FALSE}
mkdir downloads
mkdir HsRAMPAGEbam
```
...
```{bash eval=FALSE}
#The following are executed in your shell terminal:
mv *.bam downloads
cd HsRAMPAGEbam
ln -s ../downloads/ENCFF214GWH.bam H460-rep1.bam
ln -s ../downloads/ENCFF265SGZ.bam H460-rep2.bam
ln -s ../downloads/ENCFF474YPI.bam HT1080-rep1.bam
ln -s ../downloads/ENCFF242UWH.bam HT1080-rep2.bam
cd ..
```

We'll also need to download a human gene annotation, which will be important a little later.

Please download and uncompress the annotation file.
...
To implement parallelization, we'll need to set up a cluster of nodes first (here we use 4 nodes as we have 4 .bam files):
...
To check this on the tssObject S4 object you have created, simply check the list of .bam files as follows:
...
```{r eval=FALSE}
setSampleID(experimentName=Hs_RAMPAGE, sample.names=c("H460-rep1", "H460-rep2","HT1080-rep1","HT1080-rep2"), replicate.IDs=c(1,1,2,2))
```
...
Now that we have calculated the abundance for each TSS in the previous step we can calculate TSRs (promoters) on each of the 4 separate datasets.
...
As with the previous step, we select 'writeTable=TRUE', and therefore we will find the output files ("TSRset-1" to "TSRset-4") written to the working directory.
...
Now that identifying TSRs are complete, an obvious and biologically useful step is to determine which TSRs are adjacent to annotated genes, and to retrieve the appropriate gene IDs.
Before doing this, it is imperative to select an annotation file that was generated for the assembly to which the reads were aligned.
Please make sure you have already downloaded the annotation file linked to at the beginning of this vignette.
...
These data can be easily accessed from the tssObject.
For example, if we want to copy the TSR annotations from the HT1080 cells onto our R workspace, we could type the following command:
...
The annotation file is available from the TAIR10 database: ftp://ftp.arabidopsis.org/home/tair/Genes/TAIR10_genome_release/TAIR10_gff3/TAIR10_GFF3_genes.gff.
...
```{r eval=FALSE}
initializeExp(expTitle="Arabidopsis PEAT dataset", experimentName="At_PEAT", expDir="PEATbam/", isPairedEnd=TRUE)
...
```{r eval=FALSE}
importAnnotation(experimentName=At_PEAT, fileType="gff3", annotFile="TAIR10_GFF3_genes.gff")
addAnnotationToTSR(experimentName=At_PEAT, tsrSetType="replicates", tsrSet=1, upstreamDist=500, downstreamDist=200, feature="gene", featureColumnID="ID", writeTable=TRUE)
```
...
Please download and uncompress the annoation file [Note: you may ignore this is you have already downloaded the file from Example 1].
...
initializeExp(expTitle ="Human CAGE experiment", experimentName="CAGEhuman", expDir="HsCAGEbam", isPairedEnd=FALSE)
```
...
Let's take a quick glance at the 'merged" and 'combined' sets of identified TSRs:
