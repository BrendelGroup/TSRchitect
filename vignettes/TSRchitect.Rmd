...
ln -s ENCFF474YPI.bam hsRAMPAGE/HT1080-rep1.bam
ln -s ENCFF242UWH.bam hsRAMPAGE/HT1080-rep2.bam
ln -s ENCFF214GWH.bam hsRAMPAGE/H460-rep1.bam
ln -s ENCFF265SGZ.bam hsRAMPAGE/H460-rep2.bam
```
...
```{r eval=FALSE}
setSampleID(Hs_RAMPAGE, sample.names=c("HT1080-rep1","HT1080-rep2","H460-rep1", "H460-rep2"), replicate.IDs=c(1,1,2,2))
```
...
This concludes Example 1. Should we wish to save our tssObj and return to our work later, we simply type the following, which will write an R binary to your working directory.
...
Once download of the file `pead.sorted.bam` is complete, please move it to a directory within `demo/` entitled "PEAT_bam".
...
Now we import the alignment file to our tssObj S4 object and convert the data to TSS coordinates:
...
Now we have a complete set of TSRs on our tssObj object. Let's take a look at them:
...
We can optionally save the tssObj S4 as we have previously.
...
cd ../demo #assumes the user is in /vignettes
mkdir hsCAGE
ln -s ENCFF474YPI.bam hsCAGE/MCF7-rep1.bam
ln -s ENCFF242UWH.bam hsCAGE/MCF7-rep2.bam
ln -s ENCFF265RSX.bam hsCAGE/A549-rep1.bam
ln -s ENCFF944PCJ.bam hsCAGE/A549-rep2.bam
```
...
Now we can set up the tssObj S4 object. Note that we must specify `isPairedEnd=FALSE` because this is single-end CAGE data.

```{r eval=FALSE}
initializeExp(expTitle ="Human CAGE experiment", experimentName="CAGEhuman", expDir="demo/hsCAGE", isPairedEnd=FALSE) # initializing the tssObj object:
setSampleID(experimentName=CAGEhuman, sample.names=c("MCF7-rep1","MCF7-rep2","A549-rep1","A549-rep2"), replicate.IDs=c(1,1,2,2)) # setting the sample IDs and names for the datasets within the CAGE library
```

As in our other two examples, we then import the BAM files and obtain TSSs from this data.

```{r eval=FALSE}
importBam(experimentName=CAGEhuman) # loading the BAM files from CAGE experiment
bamToTSS(experimentName=CAGEhuman) # converting BAM data into TSS information and attaching it to the tssObj object:
```
...
```{r eval=FALSE}
processTSS(experimentName=CAGEhuman, parallel=TRUE, tssSet="all", writeTable=TRUE) # constructing the tag count per TSS data matrix:
determineTSR(experimentName=CAGEhuman, parallel=TRUE, tsrSetType="replicates", tssSet="all", tagCountThreshold=25, clustDist=20, writeTable=TRUE) # finding TSRs for the replicate datasets
```

Now we merge data from replicates into their two corresponding samples.

```{r eval=FALSE}
mergeSampleData(experimentName=CAGEhuman) #merging TSS data from the replicates
```

Once this is complete, we can complete TSR identification on the merged samples.

```{r eval=FALSE}
determineTSR(experimentName=CAGEhuman, parallel=TRUE, tsrSetType="merged", tssSet="all", tagCountThreshold=40, clustDist=20, writeTable=TRUE) # finding TSRs for the merged samples
```

```{r eval=FALSE}
addTagCountsToTSR(experimentName= CAGEhuman, tsrSetType= "merged", tsrSet=3, tagCountThreshold= 40, writeTable= TRUE)
```
Now we need to import the annotation file and attach it to our tssObj S4 object.
...
```{r eval=FALSE}
addAnnotationToTSR(experimentName = CAGEhuman, tsrSetType="merged", tsrSet=1, upstreamDist=1000, downstreamDist=200, feature="transcript", featureColumnID="ID", writeTable=TRUE) #MCF7 cells
addAnnotationToTSR(experimentName = CAGEhuman, tsrSetType="merged", tsrSet=2, upstreamDist=1000, downstreamDist=200, feature="transcript", featureColumnID="ID", writeTable=TRUE) #A549 cells
```
...
```{r eval=FALSE}
CAGEhuman@tsrDataMerged[[1]] -> MCF7.tsrs
dim(MCF7.tsrs)
```

```{r eval=FALSE}
CAGEhuman@tsrDataMerged[[2]] -> A549.tsrs
dim(A549.tsrs)
```
...
We can now optionally save the tssObj for future use.
